...
// static
vulkan_utils::ErrorOr<LineRenderer2> LineRenderer2::Create(
...
auto LineRenderer2::RenderFrame(VkCommandBuffer command_buffer,
  ...
  if (vulkan_line_segment_soup_) {
    ...
    vkCmdBindVertexBuffers(command_buffer, 0, 1,
                           &(vulkan_line_segment_soup_->vertex_buffer.value()),
                           &vertex_buffer_offset);
    vkCmdBindIndexBuffer(command_buffer,
                         vulkan_line_segment_soup_->index_buffer.value(), 0,
                         VK_INDEX_TYPE_UINT32);

    vkCmdDrawIndexed(
        command_buffer,
        static_cast<uint32_t>(line_segment_soup_->line_segments.size() * 2), 1,
        0, 0, 0);
  }
...
std::optional<vulkan_utils::Error> LineRenderer2::SetLineSegmentSoup(
    std::shared_ptr<const LineSegmentSoup> line_segment_soup) {
  line_segment_soup_ = line_segment_soup;
  if (!line_segment_soup_) {
    vulkan_line_segment_soup_ = nullptr;
    return std::nullopt;
  }

  size_t vertex_buffer_size = line_segment_soup_->vertices.size() *
                              sizeof(line_segment_soup_->vertices[0]);
  ...
  VULKAN_UTILS_ASSIGN_OR_RETURN(
      ...
      vulkan_utils::AllocateAndBindBufferMemory(
          ...
          reinterpret_cast<const uint8_t*>(line_segment_soup_->vertices.data()),
          vertex_buffer_size));

  size_t index_buffer_size = line_segment_soup_->line_segments.size() *
                             sizeof(line_segment_soup_->line_segments[0]);
  VULKAN_UTILS_ASSIGN_OR_RETURN(
      index_buffer,
  ...
  VULKAN_UTILS_ASSIGN_OR_RETURN(
      ...
      vulkan_utils::AllocateAndBindBufferMemory(
          ...
          reinterpret_cast<const uint8_t*>(
              line_segment_soup_->line_segments.data()),
          index_buffer_size));

  vulkan_line_segment_soup_ =
      std::shared_ptr<VulkanLineSegmentSoup>(new VulkanLineSegmentSoup{
