...
bazel --output_user_root=C:/_bzl test //src/idt/targets/pure_cpp/hashing/... //src/utils/... //src/platform_specific/... -c %BUILD_CONFIG% --symlink_prefix=/ --verbose_failures --test_output=errors
...
for /f "usebackq tokens=*" %%a in (`bazel --output_user_root=C:/_bzl info bazel-bin -c %BUILD_CONFIG%`) do (
  ...
  CALL %SCRIPT_DIR%\package_bazel_runfiles.bat %%a\ide.exe.runfiles\MANIFEST C:\build\out hypo
  CALL %SCRIPT_DIR%\package_bazel_runfiles.bat %%a\visualizer.exe.runfiles\MANIFEST C:\build\out hypo
)
