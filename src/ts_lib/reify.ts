...
export function TranslatedMesh3(
  ...
  return rci.TransformMesh3AsMesh3(rci.NewTransformMesh3(
      {source: mesh, transform: Translate3D(translation)}));
}

export function Cylinder(radius: number, thickness: number): rci.Mesh3 {
  return rci.ExtrudeMesh2AsMesh3(rci.NewExtrudeMesh2({
    source: rci.CircleAsMesh2(rci.NewCircle({radius: radius, center: [0, 0]})),
