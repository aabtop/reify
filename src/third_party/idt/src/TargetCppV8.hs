...
enumTemplate =
  case
      compileMustacheText
        "enumTemplate"
        (DT.pack  $(embedStringFile "src/CppV8EnumTemplate.stache.h"))
    of
...
structTemplate =
  case
      compileMustacheText
        "structTemplate"
        (DT.pack $(embedStringFile "src/CppV8StructTemplate.stache.h"))
    of
      Left  bundle   -> panic (errorBundlePretty bundle)
      Right template -> template
...
topLevelTemplate =
  case
      compileMustacheText
        "topLevelTemplate"
        (DT.pack $(embedStringFile "src/CppV8Template.stache.h"))
    of
      Left  bundle   -> panic (errorBundlePretty bundle)
      Right template -> template
